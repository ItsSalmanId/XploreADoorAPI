IF (OBJECT_ID('FOX_GET_INDEX_ALL_INFO') IS NOT NULL ) DROP PROCEDURE FOX_GET_INDEX_ALL_INFO  
GO                           
...
--CONVERT(BIT, ISNULL(ftis_p.IS_SYNCED, 0)) AS IS_PATIENT_INTERFACE_SYNCED,                                  
    ...
    (SELECT SUM(amt_due) FROM claims WHERE patient_account = pt.PATIENT_ACCOUNT AND isnull(claims.deleted, 0) = 0 AND pat_status IN('N', 'R', 'B', 'C', 'D')) AS Patient_Balance,                                  
         CONVERT(BIT,                                  
      CASE                                   
 ...
 END  )      
   ...
   FROM FOX_TBL_WORK_QUEUE AS wq                                    
              LEFT JOIN Patient AS pt ON wq.PATIENT_ACCOUNT = pt.Patient_Account                                    
           LEFT JOIN FOX_TBL_INTERFACE_SYNCH INS ON INS.WORK_ID = wq.WORK_ID   AND INS.CREATED_DATE = (SELECT MAX(CREATED_DATE) FROM FOX_TBL_INTERFACE_SYNCH WHERE WORK_ID = wq.WORK_ID)                                  
     LEFT JOIN FOX_TBL_INTERFACE_LOG INL on INL.FOX_INTERFACE_SYNCH_ID=INS.FOX_INTERFACE_SYNCH_ID                                
            AND ISNULL(pt.Deleted, 0) = 0                   
              LEFT JOIN WS_TBL_FOX_Temp_Patient AS wtp ON wtp.work_id = wq.work_id                                    
             LEFT JOIN FOX_TBL_ORDERING_REF_SOURCE AS s ON s.SOURCE_ID = wq.SENDER_ID                                    
              LEFT JOIN dbo.FOX_TBL_PATIENT AS ftp ON pt.Patient_Account = ftp.Patient_Account                                    
              LEFT JOIN FOX_TBL_INTERFACE_SYNCH AS ftis ON @WORK_ID IS NOT NULL                                    
    ...
    AND ISNULL(OC.DELETED, 0) = 0                              
              LEFT JOIN FOX_TBL_INTERFACE_SYNCH AS ftis_p ON ftis_p.FOX_INTERFACE_SYNCH_ID =                                    
         (                                    
