...
BEGIN      
 ...
 IF ISNULL(@PAT_DIAG_ID,0) <> 0 AND ISNULL(@PATIENT_ACCOUNT,0) <> 0 AND EXISTS (SELECT TOP 1 * FROM FOX_TBL_PATIENT_DIAGNOSIS WHERE PAT_DIAG_ID = @PAT_DIAG_ID AND ISNULL(DELETED,0) = 0)      
 BEGIN      
 -- UPDATE HERE      
