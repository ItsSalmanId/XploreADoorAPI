...
--,@PATIENT_ACCOUNT BIGINT   = 101271453412189                                                                        
--,@CASE_ID BIGINT  = NULL                                                                        
--,@OPTION  VARCHAR(20)  = 'CLOSED'                                                
--,@USER_ID BIGINT  = null                                        
--,@CURRENT_PAGE INT  = 1                                                
...
--,@DATE_FROM DATETIME=  '2000-10-15 03:22:44.470'                                    --,@DATE_TO DATETIME=  '2022-12-15 03:22:44.470'                      
--,@OWNER_ID        BIGINT= 5651352--02MTB                                                           
--,@MODIFIED_BY VARCHAR(70)=''                                          
--,@IS_USER_LEVEL bit = 0                                 
BEGIN                                                        
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------                                  
 ...
 OR (T.IS_SEND_TO_USER = 0 AND T.SEND_TO_ID IN (SELECT GROUP_ID FROM FOX_TBL_GROUP with (nolock) where ISNULL(DELETED,0)= 0 AND PRACTICE_CODE = @PRACTICE_CODE))                    )  
    )              
                   ...
                   AND ((@OPTIION = 'ALL' AND ((@IS_USER_LEVEL = 1 AND T.IS_SENDTO_MARK_COMPLETE <> 1) or (@IS_USER_LEVEL = 0 AND (T.CREATED_BY = @USER_NAME OR T.CREATED_BY <> @USER_NAME OR  (T.CREATED_BY = @USER_NAME AND ISNULL(IS_FINALROUTE_MARK_COMPLETE, 0) = 1)))))        
