IF (OBJECT_ID('FOX_PROC_GET_TASK_DETAIL_LIST') IS NOT NULL ) DROP PROCEDURE FOX_PROC_GET_TASK_DETAIL_LIST  
GO 
...
END                  
SELECT @USER_NAME = USER_NAME FROM FOX_TBL_APPLICATION_USER WHERE USER_ID = @USERID;                                                            
SELECT @OWNER_NAME = USER_NAME FROM FOX_TBL_APPLICATION_USER WHERE USER_ID = @OWNER_ID;                                                              
IF (@MODIFIED_BY = '')                                
BEGIN                                
...
FROM FOX_TBL_TASK AS T_I      
                  INNER JOIN FOX_TBL_TASK_TYPE AS TT_I ON TT_I.TASK_TYPE_ID = T_I.TASK_TYPE_ID        
                                                          AND ISNULL(TT_I.DELETED, 0) = 0        
                                                          AND TT_I.PRACTICE_CODE = @PRACTICECODE      
                  INNER JOIN FOX_TBL_TASK_TASK_SUB_TYPE AS TTST_I ON TTST_I.TASK_ID = T_I.TASK_ID     
                                                          AND ISNULL(TTST_I.DELETED, 0) = 0      
                                                          AND TTST_I.PRACTICE_CODE = @PRACTICECODE      
                  INNER JOIN FOX_TBL_TASK_SUB_TYPE AS TST_I ON TST_I.TASK_SUB_TYPE_ID = TTST_I.TASK_SUB_TYPE_ID       
                                                          AND TST_I.TASK_TYPE_ID = TT_I.TASK_TYPE_ID       
                                                          AND ISNULL(TST_I.DELETED, 0) = 0           
                                                          AND TST_I.PRACTICE_CODE = @PRACTICECODE        
             WHERE ISNULL(T_I.DELETED, 0) = 0       
                   AND T_I.PRACTICE_CODE = @PRACTICECODE    
...
SELECT DISTINCT ', '+T_I.[NAME] FROM  #a as T_I     
         ...
         FROM FOX_TBL_TASK AS T                                        
              INNER JOIN FOX_TBL_TASK_TYPE AS TT ON TT.TASK_TYPE_ID = T.TASK_TYPE_ID                        
             ...
             AND ISNULL(TTST.DELETED, 0) = 0                                        
              ...
              INNER JOIN FOX_TBL_TASK_SUB_TYPE AS TST ON TST.TASK_SUB_TYPE_ID = TTST.TASK_SUB_TYPE_ID                                        
         ...
         WHERE ISNULL(T.DELETED, 0) = 0                                        
               AND T.PRACTICE_CODE = @PRACTICECODE                                        
      GROUP BY T.TASK_ID;                          
         --                  
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------                  
   SELECT T.TASK_ID,                                         
     ...
     isnull(T.IS_SENDTO_MARK_COMPLETE,0) as IS_SENDTO_MARK_COMPLETE,                  
              ...
              --      THEN                                       
              --         CASE                                        
              --         WHEN ISNULL(final_route_user.LAST_NAME, '') = ''                        
              --         THEN ISNULL(final_route_user.FIRST_NAME, '')                                        
              --         ELSE ISNULL(final_route_user.LAST_NAME, '')+', '+ISNULL(final_route_user.FIRST_NAME, '')                         
              --         END                                        
                                                    
              --      ELSE  final_route_group.GROUP_NAME                                      
              --     END AS FINAL_ROUTE,                                      
   ...
   WHEN ISNULL(FP.LAST_NAME, '') = ''                                        
     ...
     (select count(TASK_LOG_ID) from FOX_TBL_TASK_LOG log WHERE log.TASK_ID =  T.TASK_ID and ISNULL(DELETED,0) = 0 AND log.PRACTICE_CODE = @PRACTICECODE) AS NO_OF_TIMES_MODIFIED,                                    
     ISNULL(P.CHART_ID  ,'') AS MRN                  
  INTO #TASK_DATA                  
             FROM FOX_TBL_TASK T                                        
                  LEFT JOIN FOX_TBL_CASE CS ON T.CASE_ID = CS.CASE_ID                                        
     LEFT JOIN FOX_TBL_PROVIDER FP ON T.PROVIDER_ID = FP.FOX_PROVIDER_ID                                        
                  LEFT JOIN FOX_TBL_APPLICATION_USER AU ON T.CREATED_BY = AU.USER_NAME          
              AND ISNULL(AU.DELETED, 0) = 0          
                  ...
                  LEFT JOIN PATIENT P ON T.PATIENT_ACCOUNT = P.PATIENT_ACCOUNT                                        
                  LEFT JOIN #TASK_SUB_TYPES AS tst ON tst.TASK_ID = T.TASK_ID                                   
                  LEFT JOIN FOX_TBL_PATIENT_POS PPOS ON PPOS.LOC_ID = T.LOC_ID                                        
                  ...
                  LEFT JOIN FOX_TBL_APPLICATION_USER send_to_user ON t.SEND_TO_ID = send_to_user.USER_ID                                      
                                                          AND ISNULL(send_to_user.DELETED, 0) = 0          
                          
                --  LEFT JOIN FOX_TBL_APPLICATION_USER final_route_user ON t.FINAL_ROUTE_ID = final_route_user.USER_ID                                        
                --AND ISNULL(final_route_user.DELETED, 0) = 0                                        
                  LEFT JOIN FOX_TBL_GROUP send_to_group ON t.SEND_TO_ID = send_to_group.GROUP_ID                                      
                AND ISNULL(send_to_group.DELETED, 0) = 0          
    AND send_to_group.PRACTICE_CODE = @PRACTICE_CODE          
    --              LEFT JOIN FOX_TBL_GROUP final_route_group ON t.FINAL_ROUTE_ID = final_route_group.GROUP_ID                                       
    --            AND ISNULL(final_route_group.DELETED, 0) = 0          
    --AND final_route_group.PRACTICE_CODE = @PRACTICE_CODE          
                  LEFT JOIN FOX_TBL_APPLICATION_USER AU_MODIFIED ON T.MODIFIED_BY = AU_MODIFIED.USER_NAME                                                       
                  LEFT JOIN FOX_TBL_PATIENT_INSURANCE INS  ON INS.Patient_Account = T.Patient_Account                   
                AND ISNULL(INS.DELETED, 0) = 0                    
                AND INS.Pri_Sec_Oth_Type='P'                   
                AND INS.FOX_INSURANCE_STATUS = 'C'                                      
               LEFT JOIN FOX_TBL_ORDERING_REF_SOURCE ORS ON CS.CERTIFYING_REF_SOURCE_ID = ORS.SOURCE_ID                   
       ...
       FROM FOX_TBL_task_TASK_SUB_TYPE                   
       WHERE                   
       ISNULL(DELETED,0) = 0          
              AND send_to_user.PRACTICE_CODE = @PRACTICE_CODE          
     AND AU.PRACTICE_CODE = @PRACTICE_CODE          
                 ...
                 AND TASK_SUB_TYPE_ID = @TASK_SUB_TYPE_ID) > 0)                                        
                   ...
                   AND ((@OWNER_ID IS NULL OR T.SEND_TO_ID = @OWNER_ID  or T.SEND_TO_ID IN (SELECT GROUP_ID FROM FOX_TBL_USER_GROUP WHERE USER_NAME = @OWNER_NAME AND PRACTICE_CODE = @PRACTICE_CODE AND ISNULL(DELETED,0)= 0)) or ((@OPTIION = 'OPEN SENT' or
 ...
 = @USER_NAME)) --OR  T.FINAL_ROUTE_ID = @OWNER_ID            
       ...
       OR (T.IS_SEND_TO_USER = 0 AND T.SEND_TO_ID IN (SELECT GROUP_ID FROM FOX_TBL_GROUP where ISNULL(DELETED,0)= 0 AND PRACTICE_CODE = @PRACTICE_CODE) AND ISNULL(IS_SENDTO_MARK_COMPLETE, 0) = 0)                            
            
                       --OR (T.IS_FINAL_ROUTE_USER = 1 AND T.FINAL_ROUTE_ID = @USERID AND ISNULL(IS_FINALROUTE_MARK_COMPLETE, 0) = 0)                                        
                       --OR (T.IS_FINAL_ROUTE_USER = 0 AND T.FINAL_ROUTE_ID IN (SELECT GROUP_ID FROM FOX_TBL_USER_GROUP WHERE USER_NAME = @USER_NAME  AND ISNULL(DELETED,0)= 0) AND ISNULL(IS_FINALROUTE_MARK_COMPLETE, 0) = 0)                  
       )                  
        )                                        
                    --OR (                  
        --@OPTIION = 'CLOSED'                                        
