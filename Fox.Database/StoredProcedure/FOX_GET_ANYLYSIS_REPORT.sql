IF (OBJECT_ID('FOX_GET_ANYLYSIS_REPORT') IS NOT NULL ) DROP PROCEDURE FOX_GET_ANYLYSIS_REPORT  
GO     
-- --FOX_GET_ANYLYSIS_REPORT  '','','YEARTODATE','','','','',1,1,100     
CREATE PROCEDURE FOX_GET_ANYLYSIS_REPORT                 
     
     
-- DECLARE    
--@DATEFROM VARCHAR(15) ='',    
--@DATETO VARCHAR(15) ='',    
--@TIMEFRAME VARCHAR(50) ='YEARTODATE',    
...
FOX_TBL_REFERRAL_ASSIGNMENT_DETAILS WHERE WORK_ID=MAIN.WORK_ID    
...
GROUP BY A.ROLE_NAME,RECEIVE_DATE                  
 ...
 ----------------INDXER ASSIGN TIME---------------------------------                  
 SELECT D.WORK_ID,W.RECEIVE_DATE,D.CREATED_DATE INTO   #INDEX_TIMEDATA1 FROM FOX_TBL_REFERRAL_ASSIGNMENT_DETAILS D JOIN           
...
WHERE RECEIVE_DAY <>'SATURDAY'   AND RECEIVE_DAY <>'SUNDAY'  )                  
 ...
 from #PAGINATION9 WHERE RECEIVE_DAY <>'SATURDAY'   AND RECEIVE_DAY <>'SUNDAY'                  
 AND ROW BETWEEN( @PAGEINDEX -1) *  @PAGESIZE  + 1 AND(((  @PAGEINDEX  -1) *   @PAGESIZE  + 1) +   @PAGESIZE ) - 1                   
 OR ROW=1                  
  ORDER BY ROW                   
END                  
