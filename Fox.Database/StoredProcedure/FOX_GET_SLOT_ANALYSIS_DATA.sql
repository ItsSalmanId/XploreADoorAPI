...
FROM FOX_TBL_WORK_QUEUE  with (nolock)  Where  
...
JOIN  FOX_TBL_APPLICATION_USER R with (nolock)  ON R.USER_NAME = W.ASSIGNED_TO   
LEFT JOIN FOX_TBL_ROLE E  with (nolock) ON E.ROLE_ID =R.ROLE_ID  
WHERE COMPLETED_DATE IS NOT NULL    
AND ISNULL(W.DELETED,0)=0   
AND W.WORK_STATUS=UPPER('COMPLETED')  
  
SELECT ASSIGNED_BY_DESIGNATION,WORK_ID,DELETED,  
(CONVERT(BIGINT, DATEDIFF(SS,CONVERT(DATETIME,(SELECT TOP 1 ASSIGNED_TIME FROM FOX_TBL_REFERRAL_ASSIGNMENT_DETAILS with (nolock) WHERE WORK_ID=MAIN.WORK_ID AND ASSIGNED_TIME < MAIN.ASSIGNED_TIME  
ORDER BY ASSIGNED_TIME DESC)), CONVERT(DATETIME,ASSIGNED_TIME)))) TOTALTIMEINSECONDS  INTO #LOGQUEUE_DATA  
FROM FOX_TBL_REFERRAL_ASSIGNMENT_DETAILS  AS MAIN  with (nolock) 
...
select min(CREATED_DATE) as CREATED_DATE, WORK_ID into #SupervisorData from FOX_TBL_REFERRAL_ASSIGNMENT_DETAILS with (nolock) 
...
select s.*,u.FIRST_NAME+' '+u.LAST_NAME as NAME into #SupervisorDataFinal from #SupervisorData s left join  FOX_TBL_REFERRAL_ASSIGNMENT_DETAILS r  with (nolock) on s.CREATED_DATE = r.CREATED_DATE  
left join FOX_TBL_APPLICATION_USER  u  with (nolock) on u.USER_NAME = r.ASSIGNED_TO  
...
(select FIRST_NAME +' '+LAST_NAME  NAME from FOX_TBL_APPLICATION_USER  with (nolock) where USER_NAME = ASSIGNED_TO)  ASSIGNED_TO  
,ASSIGNED_BY  
,Convert(varchar,CONVERT(DateTime,ASSIGNED_DATE),101 ) +' '+  Convert(varchar,CONVERT(DateTime,ASSIGNED_DATE),108 ) AS ASSIGNED_DATE  
,(select FIRST_NAME +' '+LAST_NAME  NAME from FOX_TBL_APPLICATION_USER  with (nolock) where USER_NAME = COMPLETED_BY) COMPLETED_BY  
...
,DOCUMENT_TYPE,(select NAME from fox_tbl_document_type  with (nolock)  where DOCUMENT_TYPE = DOCUMENT_TYPE_ID) as DOCUMENT_NAME ,SENDER_ID,FACILITY_NAME,DEPARTMENT_ID,IS_EMERGENCY_ORDER,REASON_FOR_VISIT,ACCOUNT_NUMBER,UNIT_CASE_NO,CREATED_BY,MODIFIED_BY  
...
select min(CREATED_DATE) as CREATED_DATE, WORK_ID into #SupervisorDataE from FOX_TBL_REFERRAL_ASSIGNMENT_DETAILS  with (nolock)   
where Upper(ASSIGNED_TO_DESIGNATION) = 'Supervisor'   
and isnull(deleted,0) = 0 group by WORK_ID  
  
IF OBJECT_ID('TEMPDB.DBO.#SupervisorDataFinal_E', 'U') IS NOT NULL DROP TABLE  #SupervisorDataFinal_E  
select s.*,u.FIRST_NAME+' '+u.LAST_NAME as NAME into #SupervisorDataFinal_E from #SupervisorDataE s left join  FOX_TBL_REFERRAL_ASSIGNMENT_DETAILS r  with (nolock) on s.CREATED_DATE = r.CREATED_DATE  
left join FOX_TBL_APPLICATION_USER  u  with (nolock) on u.USER_NAME = r.ASSIGNED_TO  
and isnull(u.deleted,0) = 0   
...
,Convert(varchar,CONVERT(DateTime,RECEIVE_DATE),101 ) +' '+  Convert(varchar,CONVERT(DateTime,RECEIVE_DATE),108 ) AS RECEIVE_DATE  
,TOTAL_PAGES,NO_OF_SPLITS,FILE_PATH,  
(select top 1 FIRST_NAME +' '+LAST_NAME  NAME from FOX_TBL_APPLICATION_USER  with (nolock)  where USER_NAME = ASSIGNED_TO)  ASSIGNED_TO,  
ASSIGNED_BY  
,Convert(varchar,CONVERT(DateTime,ASSIGNED_DATE),101 ) +' '+  Convert(varchar,CONVERT(DateTime,ASSIGNED_DATE),108 ) AS ASSIGNED_DATE  
,(select top 1  FIRST_NAME +' '+LAST_NAME  NAME from FOX_TBL_APPLICATION_USER  with (nolock) where USER_NAME = COMPLETED_BY) COMPLETED_BY  
,Convert(varchar,CONVERT(DateTime,COMPLETED_DATE),101 ) +' '+  Convert(varchar,CONVERT(DateTime,COMPLETED_DATE),108 ) AS COMPLETED_DATE  
,INDEXED_BY   
, Convert(varchar,CONVERT(DateTime,INDEXED_DATE),101 ) +' '+  Convert(varchar,CONVERT(DateTime,INDEXED_DATE),108 ) AS INDEXED_DATE  
,DOCUMENT_TYPE,(select top 1  NAME from fox_tbl_document_type  with (nolock) where DOCUMENT_TYPE = DOCUMENT_TYPE_ID) as DOCUMENT_NAME ,SENDER_ID,FACILITY_NAME,DEPARTMENT_ID,IS_EMERGENCY_ORDER,REASON_FOR_VISIT,ACCOUNT_NUMBER,UNIT_CASE_NO,CREATED_BY,MODIFIED_BY  
, Convert(varchar,CONVERT(DateTime,TRANSFER_DATE),101 ) +' '+  Convert(varchar,CONVERT(DateTime,TRANSFER_DATE),108 ) AS TRANSFER_DATE  
,supervisor_status,FAX_ID  
, Convert(varchar,CONVERT(DateTime,INDEXER_ASSIGN_DATE),101 ) +' '+  Convert(varchar,CONVERT(DateTime,INDEXER_ASSIGN_DATE),108 ) AS INDEXER_ASSIGN_DATE  
, Convert(varchar,CONVERT(DateTime,AGENT_ASSIGN_DATE),101 ) +' '+  Convert(varchar,CONVERT(DateTime,AGENT_ASSIGN_DATE),108 ) AS AGENT_ASSIGN_DATE  
