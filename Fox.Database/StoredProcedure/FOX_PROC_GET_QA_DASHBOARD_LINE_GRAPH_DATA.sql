...
DECLARE @d1 DATETIME, @d2 DATETIME, @year bigint,@PERCENTAGE_VALUE INT, @PERCENTAGE_VALUE1 INT             
...
SELECT @d1 = @START_DATE ,@d2= @END_DATE, @Year = YEAR(CAST(@d1 AS DATE))          
;WITH dates ( [date] )          
AS          
(          
SELECT @d1          
...
select  DISTINCT DATEPART(week,[date]) AS WEEK_NUMBER          
   ,convert(varchar(50), isnull(M.WeekStart,DATEADD(wk,DATEDIFF(wk, 5,CAST(RTRIM(@year * 10000 + 1 * 100 + 1) AS DATETIME)) + (DATEPART(week,[date]) - 1 ), 0)),10) + '  to  '          
   +convert(varchar(50), isnull(m.WeekEnd,DATEADD(second, -1,DATEADD(day,DATEDIFF(day, 0,DATEADD(wk,DATEDIFF(wk, 5,CAST(RTRIM(@year * 10000+ 1 * 100 + 1) AS DATETIME)) + (DATEPART(week,[date]) + -1 ), 5)) + 1, 1))),10) as DATE_RANGE          
...
left join FOX_TBL_PHD_CALL_SCENARIO cs on cs.PHD_CALL_SCENARIO_ID = ps.PHD_CALL_SCENARIO_ID        
left join fox_tbl_application_user au on au.user_name = ps.AGENT_NAME    left join FOX_TBL_EVALUATION_CRITERIA ec on ec.CALL_TYPE = ps.CALL_TYPE          
WHERE          
