...
BEGIN                    
 IF(@CALL_TYPE = 'PHD')                  
  ...
  FROM FOX_TBL_EVALUATION_CRITERIA WHERE PRACTICE_CODE = @PRACTICE_CODE AND ISNULL(DELETED, 0) = 0 AND CALL_TYPE = @CALL_TYPE AND CRITERIA_NAME = @CRITERIA_NAME)              
  ...
  FROM FOX_TBL_EVALUATION_CRITERIA WHERE PRACTICE_CODE = @PRACTICE_CODE AND ISNULL(DELETED, 0) = 0 AND CALL_TYPE = @CALL_TYPE AND CRITERIA_NAME = 'Client Experience')               
  SET @SYSTEM_PERCENTAGE_VALUE = (SELECT (PERCENTAGE * 0.75) AS PERCENTAGE_VALUE               
  FROM FOX_TBL_EVALUATION_CRITERIA WHERE PRACTICE_CODE = @PRACTICE_CODE AND ISNULL(DELETED, 0) = 0 AND CALL_TYPE =  @CALL_TYPE AND CRITERIA_NAME = 'System Product and Process')            
   SELECT               
...
IF(@CRITERIA_NAME = 'System Product and Process')              
 ...
 IF(@CALL_TYPE = 'Phd and Survey')                  
  ...
  AND             
  (@AGENT_NAME IS NULL OR AGENT_NAME IN (SELECT VAL FROM DBO.F_SPLIT(@AGENT_NAME, ',')))                       
  AND CONVERT(DATE, MODIFIED_DATE) BETWEEN CONVERT(DATE, @START_DATE) AND CONVERT(DATE, @END_DATE)                        
  AND PRACTICE_CODE = @PRACTICE_CODE                          
  AND ISNULL(DELETED,0) = 0                           
 ( SELECT             
