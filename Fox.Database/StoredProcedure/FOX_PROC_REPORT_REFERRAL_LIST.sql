...
left join fox_tbl_document_type dt WITH (NOLOCK) on dt.DOCUMENT_TYPE_ID=wq.DOCUMENT_TYPE and ISNULL(dt.DELETED, 0) = 0
LEFT JOIN FOX_TBL_APPLICATION_USER AT WITH (NOLOCK) ON WQ.ASSIGNED_TO = AT.USER_NAME and ISNULL(at.DELETED, 0) = 0
LEFT JOIN FOX_TBL_APPLICATION_USER CB WITH (NOLOCK) ON WQ.COMPLETED_BY = CB.USER_NAME and ISNULL(cb.DELETED, 0) = 0
LEFT JOIN FOX_TBL_APPLICATION_USER IB WITH (NOLOCK) ON WQ.ASSIGNED_BY = IB.USER_NAME and ISNULL(ib.DELETED, 0) = 0
LEFT JOIN FOX_TBL_ROLE TR WITH (NOLOCK) ON AT.ROLE_ID = TR.ROLE_ID and ISNULL(tr.DELETED, 0) = 0
